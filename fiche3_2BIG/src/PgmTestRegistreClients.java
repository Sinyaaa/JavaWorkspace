...
public class PgmTestRegistreClients {
    public static void main(String Args[]) throws Exception {
        ...
        registreClient.add(client1);
        registreClient.add(client2);

        Iterator<Client> iterateurClient = registreClient.iterator();

        while(iterateurClient.hasNext()){
            Client client = iterateurClient.next();
            System.out.println(client.getPrenom() + " " + client.getNom());
        }

        iterateurClient = registreClient.iterator();

        while(iterateurClient.hasNext()){
            iterateurClient.next();
            iterateurClient.remove();
        }
        if (registreClient.isEmpty()){
            System.out.println("Les clients on été supprimés");
        } else {
            System.out.println("ERREUR CLIENT(S) ENCORE PRESENT");
        }
